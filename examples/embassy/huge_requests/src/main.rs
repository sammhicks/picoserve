...
impl picoserve::routing::RequestHandlerService<()> for CalculateHash {
    ...
    >(
        ...
        let hash = loop {
            ...
            if last_log_time.elapsed() > embassy_time::Duration::from_secs(1) {
                last_log_time = embassy_time::Instant::now();
        ...
        format_args!(
            "SHA2 hash: {}\r\n",
            core::fmt::from_fn(|f| { hash.iter().try_for_each(|b| write!(f, "{b:02x}")) })
        )
        .write_to(request.body_connection.finalize().await?, response_writer)
        .await
    }
