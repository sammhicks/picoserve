...
type Message = heapless::String<32>;

static MESSAGE: embassy_sync::watch::Watch<
...
struct Messages {
    reciever: embassy_sync::watch::DynReceiver<'static, Message>,
}

impl picoserve::response::sse::EventSource for Messages {
    async fn write_events<W: picoserve::io::Write>(
        ...
        loop {
            match embassy_time::with_timeout(
                ...
                self.reciever.changed(),
            )
            .await
            {
                Ok(message) => {
                    writer
                        .write_event("message_changed", message.as_str())
                        .await?
                }
                Err(embassy_time::TimeoutError) => writer.write_keepalive().await?,
            }
        }
    }
}
...
impl AppBuilder for AppProps {
    ...
    fn build_app(self) -> picoserve::Router<Self::PathRouter> {
        ...
        .route(
            "/set_message",
            post(async move |message| MESSAGE.sender().send(message)),
        )
        .route(
            "/events",
            get(async move || {
                MESSAGE
                    .dyn_receiver()
                    .map(|reciever| picoserve::response::EventStream(Messages { reciever }))
                    .ok_or((
